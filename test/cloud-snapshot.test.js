...
describe('cloud snapshot: envelope', () => {
    ...
    test('refuses a tampered header', () => {
        const sealed = backup.sealWithKey(PAYLOAD, KEY);
    ...
    test('will not open a passphrase backup as a snapshot', () => {
...
describe('cloud snapshot: contents', () => {
    test('includes the hosts the user made', () => {
        assert.ok(collected.hosts.some(h => h.id === 'mine-1'), 'the user host is missing');
    });

    test('carries credentials so another device can connect', () => {
    ...
    test('includes folders, keys and snippets', () => {
    ...
    test('carries terminal settings once the renderer has reported them', () => {
        snapshot.setSettings({ 'terminal.appearance': '{"fontSize":14}', theme: 'dark' });
        assert.strictEqual(snapshot.collect().settings.theme, 'dark');
    });

    test('a collected snapshot survives the round trip intact', () => {
...
describe('cloud snapshot: guards', () => {
    test('reports being disconnected rather than uploading nothing', async () => {
    ...
    test('reports being locked rather than uploading nothing', async () => {
        const connected = syncConnectionStub.status;
...
describe('cloud snapshot: push and pull', () => {
    test('pushes to the server', async () => {
        const result = await snapshot.push();
    ...
    test('pulls back what it pushed', async () => {
    ...
    test('restores the credentials with it', () => {
     ...
     */
    test('keeps the folder tree, placement and manual order', async () => {
     ...
     */
    test('a different connection still restores after disconnecting', async () => {
    ...
    test('a corrupted payload is reported, not applied', async () => {
    ...
    test('a push with no key available reports rather than throwing', async () => {
