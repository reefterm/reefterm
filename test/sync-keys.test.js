...
describe('sync-keys: registration', () => {
    test('creates a 32-byte key and two envelopes wrapping it', () => {
...
describe('sync-keys: unlocking', () => {
    test('unlocks with the passphrase', () => {
    ...
    test('unlocks with the recovery code', () => {
    ...
    test('the recovery code still works normalized (no hyphens, lowercase)', () => {
        const { smk, recoveryEnvelope, recoveryCode } = syncKeys.createAccount('a good passphrase');
    ...
    test('a wrong passphrase fails exactly like a tampered envelope: null, not a thrown error', () => {
    ...
    test('a wrong recovery code fails the same way', () => {
    ...
    test('the passphrase envelope does not open with the recovery code, or vice versa', () => {
...
describe('sync-keys: changing the passphrase', () => {
    test('resealing under a new passphrase opens with the new one', () => {
    ...
    test('the old passphrase no longer opens the resealed envelope', () => {
        const { smk } = syncKeys.createAccount('the old passphrase');
        const resealed = syncKeys.reseal(smk, 'the new passphrase');
        assert.strictEqual(syncKeys.unsealSmk(resealed, 'the old passphrase'), null);
    });

    test('changing the passphrase leaves the recovery code working, unrelated to it', () => {
    ...
    test('rejects resealing under a too-short passphrase', () => {
        const { smk } = syncKeys.createAccount('a good passphrase');
        assert.throws(() => syncKeys.reseal(smk, 'no'));
    });
});

describe('sync-keys: rotating the recovery code', () => {
    test('rotation produces a new code that opens the same key', () => {
        const { smk } = syncKeys.createAccount('a good passphrase');
    ...
    test('the old recovery code does not open the rotated envelope', () => {
...
describe('sync-keys: local cache', () => {
    test('caches and reloads the key', () => {
        const { smk } = syncKeys.createAccount('a good passphrase');
    ...
    test('reports no cached key when none was ever cached', () => {
    ...
    test('the cache file on disk never contains the key in the clear', () => {
        const { smk } = syncKeys.createAccount('a good passphrase');
        syncKeys.cacheSmk(smk);
